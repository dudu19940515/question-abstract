...
from pgn_tf2.model_layers import Encoder, Decoder, Pointer, BahdanauAttention, ReduceState
...
class PGN(tf.keras.Model):
    def __init__(self, params):
        ...
        self.decoder = Decoder(self.embedding_matrix,
                               ...
                               params["attn_units"],
                               params["batch_size"])
        self.reduce_state = ReduceState(params["enc_units"])
        self.pointer = Pointer()

    def call_encoder(self, enc_inp):
        enc_output, enc_state = self.encoder(enc_inp)
        return enc_output, enc_state


    def call_decoder_one_step(self, dec_input, dec_state,context_vector, enc_output, enc_extended_inp,
                              batch_oov_len, enc_pad_mask,
                              use_coverage, prev_coverage):

        # context_vector, attentions, coverage_ret = self.attention(dec_state,
        #                                                           enc_output,
        #                                                           enc_pad_mask,
        #                                                           use_coverage,
        #                                                           prev_coverage)

        dec_x, pred, dec_state,context_vector, attentions, coverage_ret = self.decoder(dec_input,dec_state,
                                                                                       enc_output,enc_pad_mask,
                                                                                       context_vector,
                                                                                       use_coverage,prev_coverage)

        if self.params["pointer_gen"]:
            p_gen = self.pointer(context_vector, dec_state, dec_x)
            ...
            return tf.stack(final_dists, 1), dec_state, context_vector, attentions, p_gen, coverage_ret
        else:
            return pred, dec_state, context_vector, attentions, None, coverage_ret
    ...
    def call(self, enc_inp, dec_inp,
        ...
        # 初始上下文向量
        context_vector= tf.zeros((self.params["batch_size"], 2 *self.params["enc_units"]))
        dec_state = self.reduce_state(dec_state) #运算第一步从encoder层中传入

        # context_vector, _, coverage_ret = self.attention(dec_state,
        #                                                  enc_output,
        #                                                  enc_pad_mask,
        #                                                  use_coverage,
        #                                                  prev_coverage)
        for t in range(dec_inp.shape[1]):
            ...
            dec_x, dec_pred, dec_state, context_vector, attention, coverage_ret = self.decoder(tf.expand_dims(dec_inp[:, t], 1),dec_state,
                                                                                            enc_output,enc_pad_mask,
                                                                                            context_vector,
                                                                                            use_coverage, prev_coverage)
            # dec_x, dec_pred, dec_state = self.decoder(tf.expand_dims(dec_inp[:, t], 1),
            #                                            dec_state,
            #                                            context_vector)

            # context_vector, attn, coverage_ret = self.attention(dec_state,
            #                                                     enc_output,
            #                                                     enc_pad_mask,
            #                                                     use_coverage,
            #                                                     coverage_ret)
        ...
        if self.params["pointer_gen"]:
            ...
            if self.params["mode"] == "train":
                return final_dists, dec_state, attentions, tf.stack(coverages, 1)
            else:
                return tf.stack(final_dists, 1), dec_state, attentions, tf.stack(coverages, 1)
        else:
            return tf.stack(predictions, 1), dec_state, attentions, tf.stack(coverages, 1)
...
if __name__ == '__main__':
    ...
    params = defaultdict()
    params["max_vocab_size"] = vocab_size
    ...
    enc_output, enc_state= model.encoder(enc_inp)

    # 打印结果
    print('Encoder output shape: (batch size, sequence length, units) {}'.format(enc_output.shape))
    print('Encoder Hidden state shape: (batch size, units) {}'.format(enc_state[0].shape))

    context_vector, attention_weights, coverage = model.attention(enc_state, enc_output, enc_pad_mask)

    print("Attention context_vector shape: (batch size, units) {}".format(context_vector.shape))
    print("Attention weights shape: (batch_size, sequence_length, 1) {}".format(attention_weights.shape))

    dec_x, dec_out, dec_state = model.decoder(tf.random.uniform((64, 1)),
                                                enc_state,
                                                context_vector)
    ...
    p_gen = model.pointer(context_vector, dec_state, dec_x)
